 ...
 * 验证是否是数字(包含小数)
 */
tools.checkNumber = function (val) {
  var reg = /^\d+(\.\d+)?$/;
  var r = val.match(reg);
  if (this.isNull(r)) {
    return false;
 ...
 * 截取完整时间的时分秒部分
 */
tools.strToTime = function (val) {
  return val.split(" ")[1];
};
//截取日期部分
tools.strToYMD = function (val) {
  return val.split(" ")[0];
}
tools.strAndNum = function (val) {
  var reg = /^[a-zA-Z0-9]+$/;
...
/**
 * tooltip提示框判断条件（默认大于5提示）
 */
tools.toolTip = function (val) {
  if (!this.isNull(val) && val.length > 5) {
    return false;
  }
  return true;
};
 ...
 *   {times:100, num: 314}
 */
tools.toInteger = function (floatNum) {
 ...
 *
 */
tools.operation = function (a, b, op) {
 ...
 * @param {*} val 别除数
 * @param {*} num 除数
 * @returns true:能整除 false:不能整除
 */
tools.isNumMutiple = function (num1, num2) {
  var s1 = this.toInteger(num1);
  var s2 = this.toInteger(num2);
  var len = s2.times;
  return this.multiply(this.divide(s1.num, s1.times, 2), len) %
  this.multiply(this.divide(s2.num, s2.times), len) ==
  0
    ? true
    : false;
};
 ...
 * @param {*} number
 */
tools.getFloor = function (number) {
 ...
 * @param {*} jsonArrays
 */
tools.mergeJSON = function (jsonArrays) {

  if (jsonArrays.length < 1) {
    return {};
  }
  var json = jsonArrays[0];
